#!/usr/bin/env python3
"""Generate a tiny LoRA adapter, with nothing but the standard library.

The parent spike generates these with torch + peft, which is a heavy dependency
for what is ultimately a JSON header followed by some zeros. A safetensors file
is:

    <8 bytes little-endian header length><JSON header><raw tensor bytes>

so it is writable directly, and that keeps setup.sh self-contained.

Shaped for hmellor/tiny-random-LlamaForCausalLM: hidden_size 16, 2 layers,
q_proj/v_proj out_features 256, rank 8.

lora_B is all zeros, which is standard PEFT initialisation and a mathematical
no-op: the adapter loads and is addressable but does not change the output.
That is exactly what a routing and listing test wants -- if an adapter changed
the text, every assertion would have to account for it.

Usage:
  ./hack/gen-adapter.py OUTDIR NAME [NAME...]
"""
