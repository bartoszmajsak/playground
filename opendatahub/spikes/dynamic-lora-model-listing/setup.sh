#!/usr/bin/env bash
# Build everything this spike needs, from nothing.
#
# Creates a kind cluster with MetalLB, Gateway API, cert-manager, the inference
# extension, LWS, Istio and kserve, then stands up the on-demand LoRA fixture on
# top of it. No other spike and no pre-existing cluster required.
#
# Uses a scoped .kubeconfig next to this script; your ~/.kube/config current
# context is left alone (a kind- context is exported for convenience).
#
# The fixture, in order, because the order matters:
#
# manifests/fixture.yaml is a TEMPLATE: its route rules are an empty
# placeholder, and this script splices manifests/route-rules.yaml into them.
# Applying that file directly gives a service with no route rules.
#
#   1. namespace + PVC
#   2. seed the PVC with N tiny adapters -- through a throwaway pod, because a
...
#   ISTIO_VERSION_OVERRIDE  default 1.30.3 -- must be >= 1.29, see below
...
# Track kserve's own pinned versions rather than a set that silently drifts.
load_kserve_deps() {
    local deps
...
# Deliberately NOT tracking kserve-deps.env, and deliberately not named
# ISTIO_VERSION: load_kserve_deps exports that name (currently 1.27.1), so a
# `${ISTIO_VERSION:-1.30.3}` default silently keeps kserve's value. Istio below
# 1.29 installs the ext_proc filter as a placeholder and never attaches the
# per-route override, so the endpoint picker ends up deployed, healthy,
# resolved and never invoked -- with nothing reporting a problem.
...
# MetalLB. Every kind cluster shares the docker network, so a fixed pool is
# handed out by every cluster's MetalLB and whichever ARPs first wins. Derive
# the address from this cluster's control-plane IP, which docker keeps unique.
info "MetalLB"
...
# The route churn above writes the route several times in quick succession, and
# several routes merging on one gateway is the shape that trips istio's
# mergeHTTPRoutes data race. istiod crash-loops, its validating webhook stops
# answering, and the next apply fails with a connection refused that reads like
# a broken cluster rather than a known bug a restart clears.
istiod_ready() {
...
# Without this the ext_proc stream is reset and every pool-bound request returns
# 500 while the route, the pool and the EPP all report healthy.
